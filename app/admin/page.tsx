...
export default function PlacesAdminPage() {
  ...
  const [creating, setCreating]       = useState<Partial<Place>>({
    name: '',
    type: 'activity',
    lat: null, lng: null, maps_url: '', desc_he: '', duration_min: 60,
  });
    ...
    <main className="max-w-6xl mx-auto p-4 space-y-6">
      ...
      <section className="card space-y-3">
        <div className="grid grid-cols-3 gap-2">
          <input className="input" placeholder="חיפוש בשם/תיאור…" value={q} onChange={e => setQ(e.target.value)} />
        </div>
              ...
              <tbody>
                {filtered.map((p, i) => (
                  <tr key={p.id} className="border-t">
                    ...
                    <td className="p-2">
                      <input className="input w-56" value={p.name} onChange={e => (p.name = e.target.value)} />
                    </td>
                    <td className="p-2">
                      <input
                    ...
                    <td className="p-2">
                      <input className="input w-56" value={p.maps_url ?? ''} onChange={e => (p.maps_url = e.target.value)} />
                    </td>
                    <td className="p-2">
                      <input
                    ...
                    <td className="p-2">
                      <textarea className="input w-72" value={p.desc_he ?? ''} onChange={e => (p.desc_he = e.target.value)} />
                    </td>
                    <td className="p-2">
                ...
                {filtered.length === 0 && (
                  <tr><td className="p-2 text-gray-500" colSpan={8}>אין רשומות</td></tr>
                )}
              </tbody>
            </table>
          </div>
...
/* מחלקות עזר אם אינן קיימות ב-globals.css
...
<div className="flex gap-2">
  ...
  <Link href="/admin/places" className="btn">ניהול אטרקציות</Link>
</div>
